...
export class AuthService {
 ...
 getUserProfile(username: string, domain: string) {
  ...
  return this._basAction.New_RunAction(actionName, basParams, this._authenticationHelper.getSecurityContext).pipe(map(res=>{
  
      return this._basAction.parseToJsonobj(res, "User")
  
  }))
}
public get_secutycontext():BasSecurityContext | null {
const sec= toSignal(this.store.pipe(select(state => state?.BasSec ?? null)), { initialValue: null });
return sec()
}
